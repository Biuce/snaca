                    ...
                    <div class="container">
                        <div class="row justify-content-center">
                            <div class="col-lg-8 col-xl-5">
                                ...
                                <div class="mt-3 text-center"><img class="rounded-circle" src="/public/images/users/user-09-247x247.jpg" width="100" height="100" alt=""></div>
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.code !== 0) {
                    ...
                    layer.msg(result.msg, {skin:'alert-secondary alert-lighter'});
                    return false;
                }
                // layer.msg(result.msg, {icon: 1}, function () {
                layer.msg(result.msg, {skin:'alert-success alert-lighter'}, function () {
                    if (result.reload) {
                        location.reload();
                    }
                    if (result.redirect) {
            ...
            error: function (resp, stat, text) {
                ...
                if (resp.status === 422) {
                    ...
                    if (parse) {
                        ...
                        // layer.msg(parse.message, {shift: 6});
                        layer.msg(parse.message, {skin:'alert-secondary alert-lighter'});
                    }
                    return false;
                } else if (resp.status === 404) {
                    // layer.msg('资源不存在', {icon: 5});
                    layer.msg('资源不存在', {skin:'alert-secondary alert-lighter'});
                    return false;
                } else if (resp.status === 401) {
                    layer.msg('请先登录', {skin:'alert-secondary alert-lighter'});
                    // layer.msg('请先登录', {shift: 6});
                    return false;
                } else if (resp.status === 429) {
                    layer.msg('访问过于频繁，请稍后再试', {skin:'alert-secondary alert-lighter'});
                    // layer.msg('访问过于频繁，请稍后再试', {shift: 6});
                    return false;
                } else if (resp.status === 419) {
                    layer.msg('非法请求。请刷新页面后重试。', {skin:'alert-secondary alert-lighter'});
                    // layer.msg('非法请求。请刷新页面后重试。', {shift: 6});
                    return false;
                } else if (resp.status === 500) {
                    layer.msg('内部错误，请联系管理员', {skin:'alert-secondary alert-lighter'});
                    ...
                    // if (parse && parse.err) {
                    if (parse) {
                        layer.msg(parse.msg, {skin:'alert-secondary alert-lighter'});
